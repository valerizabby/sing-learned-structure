...
| `--temp_start` | `3.0` | Начальная температура (линейно снижается до `--temperature`) |
| `--temp_warmup_steps` | `30` | За сколько шагов снижать температуру |
| `--topk_k` | `50` | Сколько топ-нот рассматривать при сэмплировании (меньше → связнее) |
| `--n_samples` | `3` | Сколько раз сэмплировать на бит (OR → полифония) |
| `--note_extension` | `0` | Продлить каждую ноту на N битов вперёд (легато-эффект) |
...
## Рекомендуемая конфигурация для демо

Получена по результатам grid search по параметрам сэмплирования
(см. `experiments/SAMPLING_EXPERIMENTS.md`).
...
Ключевые изменения относительно дефолта и их эффект:

| Параметр | Дефолт → Демо | Эффект |
|----------|--------------|--------|
| `--temperature` | 1.5 → 1.0 | Более сфокусированный выбор нот |
| `--temp_start` | 3.0 → 1.5 | Убирает хаотичный разброс в первых шагах |
| `--topk_k` | 50 → 10 | Модель выбирает из своих реальных топ-предсказаний |
| `--n_samples` | 3 → 1 | Одна нота за бит — меньше кластеров |
| `--note_extension` | 0 → 2 | Ноты 3 бита вместо 1 — устраняет эффект стаккато |

---

## TODO

- [ ] **Подобрать SSM, отражающую следование структуре:** текущая affinity SSM задаёт
  *желаемую* структуру, но нет автоматической оценки того, насколько модель ей следует
